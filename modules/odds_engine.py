...
def analizar_apuestas(resultados, local, visita, cuotas_personalizadas=None, lineas_default=None):
    """Calcula el EV y Kelly de forma directa utilizando los números ingresados por el usuario."""
    apuestas_lista = []
    ...
    for key_m, nombre_m, prob_real, cuota in mercados_1x2:
        if cuota > 1.0:
            ...
            ev, kelly, veredicto = 0.0, 0.0, "⚠️ Ingrese cuota válida (>1.0)"
            
        apuestas_lista.append({
            "Mercado": nombre_m,
            "Prob. Modelo": f"{round(prob_real * 100, 1)}%",
            "Cuota": f"{cuota:.2f}" if cuota > 0 else "0.00",
            "EV (%)": f"{round(ev * 100, 2)}%",
    ...
    if cuota_over_goles > 1.0:
        ...
        ev_og, kelly_og, ver_og = 0.0, 0.0, "⚠️ Ingrese cuota válida (>1.0)"

    apuestas_lista.append({
        "Mercado": f"Over {l_goles} Goles",
        "Prob. Modelo": f"{round(prob_over_goles * 100, 1)}%",
        "Cuota": f"{cuota_over_goles:.2f}" if cuota_over_goles > 0 else "0.00",
    ...
    if cuota_over_corners > 1.0:
        ...
        ev_oc, kelly_oc, ver_oc = 0.0, 0.0, "⚠️ Ingrese cuota válida (>1.0)"

    apuestas_lista.append({
        "Mercado": f"Over {l_corners} Córners",
        "Prob. Modelo": f"{round(prob_over_corners * 100, 1)}%",
        "Cuota": f"{cuota_over_corners:.2f}" if cuota_over_corners > 0 else "0.00",
