...
def escanear_jornada_actual(temporada_actual=2026):
    ...
    for p in fixtures:
        # CORREGIDO: Definimos primero las variables antes de usarlas en el st.write
        fix_id = p["fixture"]["id"]
        local = p["teams"]["home"]["name"]
        visita = p["teams"]["away"]["name"]
        fecha = p["fixture"]["date"][:16].replace("T", " ")
        ...
        try:
            ...
            # Probabilidades de Montecarlo
            prob_mc_local = resultados.get("Resultado_1X2", {}).get("Gana Local", 0.0)
            prob_mc_visita = resultados.get("Resultado_1X2", {}).get("Gana Visita", 0.0)
            prob_mc_goles = resultados.get("Goles_Over_Under", {}).get("Over 2.5", 0.0)
            prob_mc_corners = resultados.get("Corners_Totales", {}).get("Over 9.5 Corners", 0.0)

            # Extraer ELO para pasarlo al ML
            try:
                elo_loc = float(tabla_posiciones_elo.loc[tabla_posiciones_elo['Equipo'] == local, 'ELO_Rating'].values[0])
                elo_vis = float(tabla_posiciones_elo.loc[tabla_posiciones_elo['Equipo'] == visita, 'ELO_Rating'].values[0])
            except:
            ...
            # Probabilidades de Machine Learning
            prob_ml_local = prob_ml_empate = prob_ml_visita = 0.0
            prob_ml_over_g = prob_ml_under_g = 0.0
            prob_ml_over_c = prob_ml_under_c = 0.0
            prob_ml_over_t = prob_ml_under_t = 0.0
            ...
            # Chivato de depuración ahora sí funcional
            if "América" in local or "Santos" in local:
                st.write(f"🔍 [DEBUG] Analizando: {local} vs {visita}")
                st.write(f"📊 prob_mc_dict recibido: {prob_mc_dict}")
                st.write(f"🤖 prob_ml_dict recibido: {prob_ml_dict}")
                st.write(f"💰 Cuotas recibidas: {cuotas}")
