...
class PredictorML:
    ...
    def _prep(self, df):
        ...
        self.features = [
            c for c in self.features if pd.api.types.is_numeric_dtype(d[c])
        ]
        ...
        d["Total_Tarjetas"] = (
            d.Amarillas_L + 2 * d.Rojas_L +
            d.Amarillas_V + 2 * d.Rojas_V
        )
    ...
    def entrenar(self, df_historico):
        ...
        # Ultimos 300 partidos: calibracion temporal fuera de entrenamiento.
    ...
    def _market_probs(pred, line, resid):
        """Convierte una regresion en distribucion empirica discreta.

        El redondeo es deliberado: goles/corners/tarjetas son conteos enteros y
        permite representar correctamente el push de lineas enteras.
        """
        if resid.size < 50:
    ...
    def predecir_mercados_completos(
        ...
        probs = dict(zip(
            self.model_1x2.classes_,
            self.model_1x2.predict_proba(X)[0]
        ))
        ...
        return {
            ...
            "Goles_Over_Under": {
                f"Over {linea_goles}": og,
                f"Under {linea_goles}": ug,
