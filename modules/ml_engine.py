...
class PredictorML:
    def __init__(self):
        # max_depth=4 y min_samples_leaf=20 son el SECRETO PROFESIONAL. 
        # Obligan a la IA a promediar la incertidumbre. NUNCA más te dará un 85%.
        self.model_1x2 = RandomForestClassifier(n_estimators=100, max_depth=4, min_samples_leaf=20, random_state=42)
        self.model_goles = RandomForestClassifier(n_estimators=100, max_depth=4, min_samples_leaf=20, random_state=42)
        self.model_corners = RandomForestClassifier(n_estimators=100, max_depth=4, min_samples_leaf=20, random_state=42)
        self.is_trained = False

    def entrenar(self, df_historico):
        """Entrena la IA utilizando el ELO previo y los promedios reales, previniendo el sobreajuste."""
        ...
        for index, row in df.iterrows():
            ...
            # Actualizar ELO para la siguiente jornada
            motor_elo.procesar_partido(loc, vis, row['Goles_L'], row['Goles_V'])

        df['Diff_ELO_Previo'] = diff_elo_lista
        
        # Promedios móviles reales de la liga (no simulaciones)
        df['Prom_Goles_L_Previo'] = df.groupby('Local')['Goles_L'].transform(lambda x: x.rolling(5, min_periods=1).mean().shift(1).fillna(1.2))
        df['Prom_Goles_V_Previo'] = df.groupby('Visitante')['Goles_V'].transform(lambda x: x.rolling(5, min_periods=1).mean().shift(1).fillna(1.2))
        
        # 3. Entrenamiento con datos limpios
    ...
    def predecir_mercados_completos(self, df_historico, equipo_local, equipo_visita, goles_sim_l, goles_sim_v, elo_local=1500, elo_visita=1500):
        """Genera predicciones con datos matemáticos aterrizados a la realidad."""
        if not self.is_trained:
            return {"1X2": {"Gana Local": 33.3, "Empate": 33.3, "Gana Visita": 33.3}, "Over_2.5_Goles": 50.0, "Over_9.5_Corners": 50.0}
        
        # IGNORAMOS los goles_sim_l (que causaban el pico de 85%) y calculamos el promedio real actual
        df_local = df_historico[df_historico['Local'] == equipo_local]
        promedio_real_l = df_local['Goles_L'].tail(5).mean() if not df_local.empty else 1.2
        
        df_visita = df_historico[df_historico['Visitante'] == equipo_visita]
        promedio_real_v = df_visita['Goles_V'].tail(5).mean() if not df_visita.empty else 1.2

        # Creamos el dataframe exactamente con la misma escala que en el entrenamiento
        df_input = pd.DataFrame([{
            'Diff_ELO_Previo': elo_local - elo_visita,
            'Prom_Goles_L_Previo': promedio_real_l, 
            'Prom_Goles_V_Previo': promedio_real_v
        }])
        
        # 1. Probabilidades 1X2 realistas
